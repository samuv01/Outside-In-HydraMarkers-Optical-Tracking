# This script is used to read and identify hydramarkers from a single image uploaded at the bottom by the user.
...
def _prepare_dot_tables(
    image: np.ndarray,
    grids: Sequence[np.ndarray],
    points: np.ndarray,
    contrast_threshold: float,
) -> Tuple[List[np.ndarray | None], List[np.ndarray | None], List[List[np.ndarray] | None], List[List[np.ndarray] | None]]:
    """
    Pre-compute dot tables and associated metadata for each grid. This allows us
    to score grids against multiple STA patterns without repeatedly sampling the
    image.
    """
    dot_tables: List[np.ndarray | None] = []
    pt_arrays: List[np.ndarray | None] = []
    yes_centers: List[List[np.ndarray] | None] = []
    no_centers: List[List[np.ndarray] | None] = []

    if image.ndim != 2:
        raise ValueError("Expected a single-channel image for dot table preparation.")
    ...
    for grid in grids:
        if grid.size == 0:
            ...
            yes_centers.append(None)
            no_centers.append(None)
            continue
        ...
        for im in range(M - 1):
            for jn in range(N - 1):
                ...
                if has_contrast:
                    dot_table[im, jn] = 1.0
                    yes_list.append(dot_center.astype(np.float64))
                else:
                    dot_table[im, jn] = 0.0
                    no_list.append(dot_center.astype(np.float64))

        dot_tables.append(dot_table)
        pt_arrays.append(pt_array)
        yes_centers.append(yes_list if yes_list else None)
        no_centers.append(no_list if no_list else None)

    return dot_tables, pt_arrays, yes_centers, no_centers


def _score_dot_table_against_sta(
    ...
    """
    Score a precomputed dot table against a specific STA pattern. Returns the best
    match metadata (score, coverage, rotation, matched array) or None if no valid
    alignment is found.
    """
        ...
        for flat_idx in order_flat:
            ...
            if coverage > best_coverage or (coverage == best_coverage and score > best_score):
                ...
                best_details = {
                    ...
                    "coverage": coverage,
                    "rotation": rot,
                    "match_position": (match_m, match_n),
                    "matched_array": array_ind,
                }
...
def pt_identify(
    ...
    dot_tables: List[np.ndarray | None]
    pt_arrays: List[np.ndarray | None]
    yes_lists_per_grid: List[List[np.ndarray] | None]
    no_lists_per_grid: List[List[np.ndarray] | None]
    ...
    if use_precomputed:
        dot_tables = list(precomputed.get("dot_tables", []))
        if len(dot_tables) < len(grids):
            dot_tables.extend([None] * (len(grids) - len(dot_tables)))

        pt_arrays = list(precomputed.get("pt_arrays", [])) if precomputed.get("pt_arrays") is not None else []
        if len(pt_arrays) < len(grids):
            pt_arrays.extend([None] * (len(grids) - len(pt_arrays)))

        yes_lists_per_grid = list(precomputed.get("yes_centers", [])) if precomputed.get("yes_centers") is not None else []
        if len(yes_lists_per_grid) < len(grids):
            yes_lists_per_grid.extend([None] * (len(grids) - len(yes_lists_per_grid)))

        no_lists_per_grid = list(precomputed.get("no_centers", [])) if precomputed.get("no_centers") is not None else []
        if len(no_lists_per_grid) < len(grids):
            no_lists_per_grid.extend([None] * (len(grids) - len(no_lists_per_grid)))

        for entry in yes_lists_per_grid:
            if entry is None:
                continue
            entry_arr = np.asarray(entry, dtype=np.float64)
            if entry_arr.ndim == 1:
                yes_dot_centers.append(entry_arr)
            else:
                for center in entry_arr:
                    yes_dot_centers.append(np.asarray(center, dtype=np.float64))

        for entry in no_lists_per_grid:
            if entry is None:
                continue
            entry_arr = np.asarray(entry, dtype=np.float64)
            if entry_arr.ndim == 1:
                no_dot_centers.append(entry_arr)
            else:
                for center in entry_arr:
                    no_dot_centers.append(np.asarray(center, dtype=np.float64))

        dot_detection_end = dot_detection_start
    else:
        dot_tables = []
        pt_arrays = []
        yes_lists_per_grid = []
        ...
        for grid in grids:
            ...
            if M < 2 or N < 2:
                dot_tables.append(None)
                pt_arrays.append(None)
                yes_lists_per_grid.append(None)
                no_lists_per_grid.append(None)
                continue
            ...
            for im in range(M - 1):
                for jn in range(N - 1):
                    ...
                    if has_contrast:
                        dot_table[im, jn] = 1.0
                        yes_center = dot_center.astype(np.float64)
                        yes_dot_centers.append(yes_center)
                        yes_list.append(yes_center)
                    else:
                        dot_table[im, jn] = 0.0
                        no_center = dot_center.astype(np.float64)
                        no_dot_centers.append(no_center)
                        no_list.append(no_center)

            dot_tables.append(dot_table)
            yes_lists_per_grid.append(yes_list if yes_list else None)
            no_lists_per_grid.append(no_list if no_list else None)
    ...
    if precomputed is None:
        precomputed_reference = {
            ...
            "pt_arrays": pt_arrays,
            "yes_centers": yes_lists_per_grid,
            "no_centers": no_lists_per_grid,
        }
    else:
        precomputed_reference = precomputed
    ...
    if return_debug:
        ...
        assignment_details = precomputed_reference.get("assignment_details")
        if assignment_details is not None:
            debug_payload["grid_assignment_details"] = assignment_details
        return result, debug_payload
    return result


def detect_corners_shared(
    image: np.ndarray,
    r: int = 5,
    expect_n: int = 500,
    sigma: float = 3.0,
    *,
    ...
    return_debug: bool = False,
) -> Tuple[RefinedPoints, List[np.ndarray], np.ndarray, dict[str, object] | None]:
    """
    Run the corner detection pipeline once so that multiple markers can share the result.
    """
    ...
    refined = pt_refine(
        ...
        detection.points,
        r=r,
        grad_x=detection.grad_x,
        grad_y=detection.grad_y,
    )
    t_after_refine = perf_counter()

    filtered = refined
    if min_cluster_points is not None:
        filtered = _filter_sparse_clusters(refined, min_cluster_points=min_cluster_points)
    t_after_cluster = perf_counter()

    grids, edges = pt_struct(filtered.points, filtered.ledges)
    t_after_struct = perf_counter()

    debug_info: dict[str, object] | None = None
    if return_debug:
        timings = {
            "corner_detection": float(max(t_after_pre - t_start, 0.0)),
            "refinement": float(max(t_after_refine - t_after_pre, 0.0)),
            "cluster_filter": float(max(t_after_cluster - t_after_refine, 0.0)),
            "struct": float(max(t_after_struct - t_after_cluster, 0.0)),
            "total": float(max(t_after_struct - t_start, 0.0)),
        }
        debug_info = {
            "corner_candidates": np.asarray(detection.points, dtype=np.float64),
            "refined_points": np.asarray(filtered.points, dtype=np.float64),
            "refined_ledges": np.asarray(filtered.ledges, dtype=np.float64),
            "response": np.asarray(detection.response, dtype=np.float64),
            "grids": list(grids),
            "edges": np.asarray(edges, dtype=np.int32),
            "timings": timings,
        }

    return filtered, grids, edges, debug_info


def identify_marker_from_shared(
    image: np.ndarray,
    sta: np.ndarray,
    refined: RefinedPoints,
    grids: Sequence[np.ndarray],
    ...
    *,
    ...
    """
    Identify a single marker using the shared corner detection results.
    """
    identify_start = perf_counter()
    pt_result = pt_identify(
        image=image,
        sta=sta,
        grids=grids,
        points=refined.points,
        ...
        id_base=id_base,
        column_major=column_major_ids,
        return_debug=return_debug,
    )
    identify_end = perf_counter()

    marker_debug: dict[str, object] | None = None
    if return_debug:
        pt_list, debug_payload = pt_result  # type: ignore[assignment]
        marker_debug = dict(debug_payload)
        timings = dict(marker_debug.get("timings", {}))
        timings["identify_total"] = float(max(identify_end - identify_start, 0.0))
        marker_debug["timings"] = timings
        marker_debug.setdefault("shared_refined_points", np.asarray(refined.points, dtype=np.float64))
        marker_debug.setdefault("shared_refined_ledges", np.asarray(refined.ledges, dtype=np.float64))
        marker_debug.setdefault("shared_grids", list(grids))
        marker_debug.setdefault("shared_edges", np.asarray(edges, dtype=np.int32))
        return pt_list, edges, marker_debug

    pt_list = pt_result  # type: ignore[assignment]
    return pt_list, edges, marker_debug


def read_marker_multi(
    image: np.ndarray,
    sta_list: Sequence[np.ndarray],
    *,
    sta_labels: Sequence[str] | None = None,
    r: int = 5,
    expect_n: int = 500,
    sigma: float = 3.0,
    contrast_threshold: float = 10,
    id_base: int = 1,
    id_bases: Sequence[int] | None = None,
    column_major_ids: bool = True,
    return_debug: bool = False,
) -> Tuple[List[Tuple[np.ndarray, np.ndarray, dict[str, object]]], dict[str, object] | None] | List[Tuple[np.ndarray, np.ndarray]]:
    """
    Process multiple markers efficiently by sharing the corner detection stage.
    Each grid is scored against every STA/rotation combination and assigned to
    the marker that yields the best score (breaking ties by coverage).
    """
    if not sta_list:
        raise ValueError("sta_list must contain at least one marker array.")

    if id_bases is not None and len(id_bases) != len(sta_list):
        raise ValueError("id_bases length must match sta_list length.")

    if sta_labels is not None and len(sta_labels) != len(sta_list):
        raise ValueError("sta_labels length must match sta_list length.")

    expected_counts = [
        (sta.shape[0] + 1) * (sta.shape[1] + 1)
        for sta in sta_list
        if sta.ndim == 2 and sta.size > 0
    ]
    min_cluster_points: int | None = None
    if expected_counts:
        min_cluster_points = max(10, int(round(0.08 * min(expected_counts))))

    refined, grids, edges, shared_debug = detect_corners_shared(
        image,
        r=r,
        expect_n=expect_n,
        sigma=sigma,
    ...
    dot_tables, pt_arrays, yes_lists_by_grid, no_lists_by_grid = _prepare_dot_tables(
        image, grids, refined.points, contrast_threshold
    )

    marker_labels = list(sta_labels) if sta_labels is not None else [f"marker_{idx + 1}" for idx in range(len(sta_list))]
    order = "F" if column_major_ids else "C"
    marker_contexts: List[dict[str, object]] = []
    for idx, sta in enumerate(sta_list):
        if sta.ndim != 2:
            raise ValueError(f"STA array at index {idx} must be 2-dimensional.")
        base = id_bases[idx] if id_bases is not None else id_base
        sta_m, sta_n = sta.shape
        id_matrix = np.arange(
            base,
            base + (sta_m + 1) * (sta_n + 1),
            dtype=float,
        ).reshape(sta_m + 1, sta_n + 1, order=order)
        marker_contexts.append(
            {
                ...
                "id_base": base,
                "id_matrix": id_matrix,
                "assigned_mask": [False] * len(grids),
                "assignment_details": [],
            }
        )

    assignment_summary: List[dict[str, object] | None] = []
    for grid_idx, (grid, dots) in enumerate(zip(grids, dot_tables)):
        if dots is None or dots.size == 0:
            assignment_summary.append(None)
            continue

        candidate_records: List[Tuple[dict[str, object], dict[str, object]]] = []
        for ctx in marker_contexts:
            candidate = _score_dot_table_against_sta(
                dots,
                grid,
                ctx["sta"],  # type: ignore[arg-type]
                ctx["id_matrix"],  # type: ignore[arg-type]
            )
            if candidate is None:
                continue
            candidate_records.append((ctx, candidate))
        ...
        candidate_records.sort(
            key=lambda item: (float(item[1]["score"]), int(item[1]["coverage"])),
            reverse=True,
        )
        best_ctx, best_candidate = candidate_records[0]

        for other_ctx, other_candidate in candidate_records[1:]:
            if math.isclose(float(other_candidate["score"]), float(best_candidate["score"]), rel_tol=1e-6, abs_tol=1e-9) and int(other_candidate["coverage"]) == int(best_candidate["coverage"]):
                print(
                    f"[WARN] tie assigning grid {grid_idx} between {best_ctx['label']} and {other_ctx['label']} "
                    f"(score={best_candidate['score']:.3f}, coverage={best_candidate['coverage']})"
                )
                break

        best_ctx["assigned_mask"][grid_idx] = True  # type: ignore[index]
        assignment_record = {
            "grid_index": int(grid_idx),
            "marker_index": int(best_ctx["index"]),  # type: ignore[index]
            "marker_label": str(best_ctx["label"]),
            "score": float(best_candidate["score"]),
            "coverage": int(best_candidate["coverage"]),
            "rotation": int(best_candidate["rotation"]),
            "match_position": tuple(int(v) for v in best_candidate["match_position"]),
        }
        if "matched_array" in best_candidate:
            assignment_record["matched_array"] = np.asarray(best_candidate["matched_array"], dtype=float)

        assignment_summary.append(assignment_record)
        best_ctx["assignment_details"].append(assignment_record)  # type: ignore[index]

        print(
            f"[assign] grid {grid_idx} -> {best_ctx['label']} "
            f"(rotation={best_candidate['rotation']}, score={best_candidate['score']:.3f}, coverage={best_candidate['coverage']})"
        )

    if return_debug:
        results_debug: List[Tuple[np.ndarray, np.ndarray, dict[str, object]]] = []
        marker_debugs: List[dict[str, object]] = []
    else:
        results_simple: List[Tuple[np.ndarray, np.ndarray]] = []

    for ctx in marker_contexts:
        assigned_mask: List[bool] = ctx["assigned_mask"]  # type: ignore[assignment]
        precomputed_data = {
            "dot_tables": [
                dot_tables[idx] if assigned_mask[idx] else None for idx in range(len(dot_tables))
            ],
            "pt_arrays": [
                pt_arrays[idx] if assigned_mask[idx] else None for idx in range(len(pt_arrays))
            ],
            "yes_centers": [
                yes_lists_by_grid[idx] if assigned_mask[idx] else None for idx in range(len(yes_lists_by_grid))
            ],
            "no_centers": [
                no_lists_by_grid[idx] if assigned_mask[idx] else None for idx in range(len(no_lists_by_grid))
            ],
            "assignment_details": list(ctx["assignment_details"]),  # type: ignore[index]
        }

        pt_output = pt_identify(
            image=image,
            sta=ctx["sta"],  # type: ignore[arg-type]
            grids=grids,
            points=refined.points,
            contrast_threshold=contrast_threshold,
            id_base=ctx["id_base"],  # type: ignore[arg-type]
            column_major=column_major_ids,
            return_debug=return_debug,
        ...
        if return_debug:
            pt_list, marker_debug = pt_output  # type: ignore[assignment]
        else:
            pt_list = pt_output  # type: ignore[assignment]
            marker_debug = None

        if edges.size:
            identified_indices = np.where(~np.isnan(pt_list[:, 2]))[0]
            if identified_indices.size:
                idx_set = set(int(i) for i in identified_indices)
                edge_mask = np.fromiter(
                    ((int(edge[0]) in idx_set) and (int(edge[1]) in idx_set) for edge in edges),
                    dtype=bool,
                    count=edges.shape[0],
                )
                marker_edges = edges[edge_mask]
            else:
                marker_edges = np.empty((0, 2), dtype=edges.dtype)
        else:
            marker_edges = np.empty((0, 2), dtype=np.int32)

        if return_debug:
            if marker_debug is None:
                marker_debug = {}
            marker_debug.setdefault("grid_assignments", list(ctx["assignment_details"]))  # type: ignore[index]
            marker_debug.setdefault("label", ctx["label"])
            marker_debug.setdefault("assigned_mask", assigned_mask)
            marker_debug.setdefault("shared_corner_debug", shared_debug)
            results_debug.append((pt_list, marker_edges, marker_debug))
            marker_debugs.append(marker_debug)
        else:
            results_simple.append((pt_list, marker_edges))

    if return_debug:
        debug_bundle = {
            "shared": shared_debug,
            "assignments": assignment_summary,
            "markers": marker_debugs,
        }
        return results_debug, debug_bundle
...
def _draw_overlay(
    ...
    *,
    ...
    if font_scale is None:
        font_scale = min(0.7, 0.1 * scale_factor)
    if font_thickness is None:
        font_thickness = min(1, int(round(scale_factor)))
    marker_radius = max(6, int(round(1 * scale_factor)))
    cross_size = max(8, int(round(8 * scale_factor)))
    ...
    text_color = (180, 120, 0) if not ambiguous else (0, 165, 255)
            ...
            for im in range(dot_table.shape[0]):
                for jn in range(dot_table.shape[1]):
                    ...
                    if np.isnan(value):
                        ...
                        cv2.circle(display, center, cell_radius, color, thickness=-2)
                    else:
                        cv2.circle(display, center, cell_radius, color, thickness=-2)

    return display

# Load the grayscale board image